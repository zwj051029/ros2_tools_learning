# ROS 2 Launch 学习示例（ament_cmake）

## 功能包说明

`learning_launch_cpp` 用于学习 ROS 2 Launch 系统，通过同一组 `turtlesim` 案例对照演示 Python、XML 和 YAML 三种启动文件格式。

功能包使用 `ament_cmake` 构建，因此名称带有 `_cpp` 后缀；当前包不编译自定义 C++ 节点，主要负责安装和管理 Launch 文件及参数配置。

对应的 `ament_python` 功能包为 `learning_launch_py`。
...
`CMakeLists.txt` 将 `launch/` 和 `config/` 安装到：

```text
install/learning_launch_cpp/share/learning_launch_cpp/
```
...
colcon build --packages-select learning_launch_cpp
...
```bash
ros2 launch learning_launch_cpp node_config_launch.py
ros2 launch learning_launch_cpp node_config_launch.xml
ros2 launch learning_launch_cpp node_config_launch.yaml
```
...
```bash
ros2 launch learning_launch_cpp command_launch.py
ros2 launch learning_launch_cpp command_launch.xml
ros2 launch learning_launch_cpp command_launch.yaml
```
...
ros2 launch learning_launch_cpp parameters_launch.xml r:=255 g:=0 b:=0
ros2 launch learning_launch_cpp parameters_launch.yaml r:=255 g:=0 b:=0
```
...
```bash
ros2 launch learning_launch_cpp include_launch.py
ros2 launch learning_launch_cpp include_launch.xml
ros2 launch learning_launch_cpp include_launch.yaml
```
...
```bash
ros2 launch learning_launch_cpp group_launch.py
ros2 launch learning_launch_cpp group_launch.xml
ros2 launch learning_launch_cpp group_launch.yaml
```
...
```bash
ros2 launch learning_launch_cpp event_launch.py
```
